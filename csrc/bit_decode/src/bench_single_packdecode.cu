...
template <int num_heads, int num_heads_kv, int head_dim, int num_bits>
double TestBatchDecodingKernelPerformance(int batch_size, int seqlen_kv, const std::string& quant_mode, const int group_size, const int repeat) {
    const int seqlen_q = 4;
    const int pack_nums = 8 / num_bits;

    torch::Tensor Q_host = torch::ones({batch_size, seqlen_q, num_heads, head_dim / pack_nums}, torch::dtype(torch::kUInt8));
    torch::Tensor Q_scale_host = torch::ones({batch_size, seqlen_q, num_heads, head_dim / 16}, torch::dtype(torch::kFloat8_e4m3fn));

    torch::Tensor K_host = torch::ones({batch_size, seqlen_kv, num_heads_kv, head_dim / pack_nums}, torch::dtype(torch::kUInt8));
    torch::Tensor K_scale_host = torch::ones({batch_size, seqlen_kv, num_heads_kv, head_dim / 16}, torch::dtype(torch::kFloat8_e4m3fn));

    torch::Tensor V_host = torch::ones({batch_size, seqlen_kv, num_heads_kv, head_dim / pack_nums}, torch::dtype(torch::kHalf));
    torch::Tensor V_scale_host = torch::ones({batch_size, seqlen_kv, num_heads_kv, head_dim / 16}, torch::dtype(torch::kFloat8_e4m3fn));
    ...
    auto cu_seqlens_k = torch::arange(0, (batch_size + 1) * seqlen_kv, seqlen_kv, torch::TensorOptions().dtype(torch::kInt32).device(torch::kCUDA));
    std::optional<at::Tensor> opt_block_table = std::nullopt;
...
int main() {
    ...
    // Single decode benchmark (original)
